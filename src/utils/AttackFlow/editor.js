...
function generateNode(
  ...
  // 使用vueRender渲染虚拟节点到newNode上
  vueRender(componentVNode, newNode);
  newNode.vNode = componentVNode; // 可选：如果你需要保持对虚拟节点的引用
...
function addNodeByAction(action, position, icon, value, type, params) {
  ...
  if (type === '数据源') {
    sourceEndpoints = [
      { id: `source-source-${createUuid()}`, data: { value: '加密算法' } },
    ]
  } else if (algorithmList.includes(type)) {
    targetEndpoints = [
      { id: `encrypt-target-${createUuid()}`, data: { value: '数据源' } },
    ]
    sourceEndpoints = [
      { id: `encrypt-source-${createUuid()}`, data: { value: '攻击算法' } },
    ]
  } else if (AttackList.includes(type)) {
    targetEndpoints = [
      { id: `attack-target-${createUuid()}`, data: { value: '加密算法' } },
    ]
  }
...
function addNodeByDrag(position, elId) {
  ...
  const hasAlgorithmNodeType = nodes.some((node) =>
    algorithmList.includes(node.data.type)
  )

  // 检查是否已经存在AttackList中的节点类型
  const hasAttackNodeType = nodes.some((node) =>
    AttackList.includes(node.data.type)
  )
...
function removeNode(nodeId) {
  ...
  const relatedPointIds = [
    ...nodeData.points.sources,
    ...nodeData.points.targets,
  ]
...
function renameNode(nodeId, value) {
  ...
  const cComp = $children(nodeEl.vNode)[0]
  cComp.text = value
...
function bindEvent() {
  ...
  instance
    .getContainer()
    .parentElement.addEventListener('contextmenu', (ev) => {
      ev.preventDefault()
      contextMenu.show({
        left: ev.pageX,
        top: ev.pageY,
  ...
  instance.bind('connection', (info, ev) => {
    ...
    if (ev) {
      ...
      if (
        (start_type === 'source' && end_type === 'encrypt') ||
        (start_type === 'encrypt' && end_type === 'attack')
      ) {
        execAddConnectorCommand(uuids)
      } else {
...
function depthFirstTraversal(rootId, nodes, edges, visited = new Set()) {
  ...
  function dfs(nodeId) {
    ...
    const connectedEdges = edges.filter((edge) =>
      edge.startsWith(`${source}&&`)
    )
    connectedEdges.forEach((edge) => {
      const target = edge.split('&&')[1]
      const targetId = nodes.find((node) =>
        node.points.targets.find((target1) => target1 === target)
      ).id
      dfs(targetId)
    })
  }
